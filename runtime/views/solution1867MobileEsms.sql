-- 1867 Scenario Candidates: Mobile Solution — ESMS Service OE
-- Returns Mobile Solutions linked to a Product Basket where ESMS is present (hasESMS or isESMSService).
-- Endpoint (after schema registration): /tmf-api/productInventory/v5/solution1867MobileEsms

DROP VIEW IF EXISTS salesforce_server."solution1867MobileEsms";
CREATE VIEW salesforce_server."solution1867MobileEsms" AS
SELECT
    ...
    ('http://bssmagic-alb-526861445.ap-southeast-1.elb.amazonaws.com/tmf-api/productInventory/v5/solution1867MobileEsms/' || t0."Id")::text AS href,
    t0."Id"::text AS id,
    'Solution1867MobileEsms'::text AS "@type",
...
WHERE t0."cssdm__product_basket__c" IS NOT NULL
  AND sd."Name" = 'Mobile Solution'
  AND (t0."hasESMS__c" = true OR t0."isESMSService__c" = true);
