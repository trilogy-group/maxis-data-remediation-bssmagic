...
--
-- validationStatus Characteristic:
--   - Always returns "valid" (EXISTS subquery removed for performance)
--   - Actual validation is done by the remediation API (validate-remigration)
--   - Previous slow query: CASE WHEN EXISTS(...) THEN 'referenced' ELSE 'valid' END
-- =====================================================================
...
SELECT
    ...
    ARRAY[
        ...
        -- validationStatus: Always "valid" (EXISTS subquery removed for performance)
        -- NOTE: Validation check removed to avoid slow FDW correlated subquery
        ROW(
            NULL::tmf."Characteristic",
            NULL::tmf."BooleanArrayCharacteristic",
            NULL::tmf."BooleanCharacteristic",
            ...
            NULL::tmf."StringArrayCharacteristic",
            ROW(
                'valid'::text,  -- Hardcoded: validation check moved to remediation API
                NULL::text,
    ...
    -- Uses ARRAY_REMOVE to filter NULL elements (TMF Server can't parse NULL in arrays)
    -- ========================================
    ARRAY_REMOVE(ARRAY[
